...
import { Moon, RefreshCw, Settings2, X, Plus, Trash2 } from "lucide-react";
...
export default function UtilityPanel() {
  ...
  useEffect(() => {
    const openUtilities = () => setIsOpen(true);
    window.addEventListener("cablesync:open-utilities", openUtilities);
    return () => window.removeEventListener("cablesync:open-utilities", openUtilities);
  }, []);
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      addToast(
        error.response?.data?.message || "Failed to reset customers",
        "error",
      );
    } finally {
      setIsBusy(false);
  ...
  return (
    <div className="utility-panel-wrapper">
      <button
        type="button"
      ...
      >
        <Settings2 className="utility-icon" />
      </button>

      {isOpen && (
        <div className="utility-panel-card">
          <div className="utility-panel-header">
            <div>
              <p className="utility-panel-title">Settings & Utilities</p>
            </div>
            <button
              type="button"
          ...
          {showConfirm && (
            <div className="utility-panel-confirm">
              ...
              <p className="utility-panel-confirm-copy">
                This will:
              </p>
